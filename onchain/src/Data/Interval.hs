...
import Utils (pletC, plt, ple, pmatchC)
...
compareEndpoints = phoistAcyclic $ plam $
...
-- endpoints with `cmp`.
unequalFiniteEndpoints :: forall (s :: S) (a :: PType) .
  (PIsData a, PEq a, POrd a) =>
  Term s (PExtended a) ->
  Term s (PExtended a) ->
  Term s (a :--> a :--> PBool) ->
...
lowerBoundLT :: forall (s :: S) (a :: PType).
  (PIsData a, PEq a, POrd a) =>
  Term s (
    PExtended a :-->
    PClosure :-->
...
lowerBoundLT = phoistAcyclic $ plam $ \p1 c1 p2 c2 ->
  ptrace "lowerBoundLT" $ ptraceIfTrue "True" $
  pmatch p1 $ \case
...
-- the two endpoints are the lower bounds of the interval. It assumes that
-- `p1` == `p2` if both are `PFinite`.
lowerBoundLE :: forall (s :: S) (a :: PType) .
  (PIsData a, PEq a, POrd a) =>
  Term s (
    PExtended a :-->
    PClosure :-->
...
lowerBoundLE = phoistAcyclic $ plam $ \p1 c1 p2 c2 ->
  ptrace "lowerBoundLE" $ ptraceIfTrue "True" $
  pmatch p1 $ \case
...
upperBoundLT :: forall (s :: S) (a :: PType) .
  (PIsData a, PEq a, POrd a) =>
  Term s (
    PExtended a :-->
    PClosure :-->
...
upperBoundLT = phoistAcyclic $ plam $ \p1 c1 p2 c2 ->
  ptrace "upperBoundLT" $ ptraceIfTrue "True" $
  pnot #$ lowerBoundLE # p1 # c1 # p2 # c2

upperBoundLE :: forall (s :: S) (a :: PType) .
  (PIsData a, PEq a, POrd a) =>
  Term s (
    PExtended a :-->
    PClosure :-->
...
upperBoundLE = phoistAcyclic $ plam $ \p1 c1 p2 c2 ->
  ptrace "upperBoundLE" $ ptraceIfTrue "True" $
...
pinterval p1 p2 = pinterval' lBound hBound
  where lBound :: Term s (PLowerBound a)
...
pintervalFrom p =
  pcon $ PInterval $
    pdcons # lBound #$
      pdcons # hBound # pdnil
  where hBound :: Term s (PAsData (PUpperBound a))
...
pintervalTo p =
  pcon $ PInterval $
    pdcons # lBound #$
      pdcons # hBound # pdnil
  where lBound :: Term s (PAsData (PLowerBound a))
...
plowerBound ::
  ...
  Term s PBool ->
  Term s (PLowerBound a)
plowerBound p closed = pcon $ PLowerBound $
...
pupperBound ::
  ...
  Term s PBool ->
  Term s (PUpperBound a)
pupperBound p closed = pcon $ PUpperBound $
  pdcons # pext p #$ pdcons # pdata closed # pdnil
...
pperiodicContains = plam $ \pi i' -> unTermCont $ do
    ...
    pure $
        ptraceIfFalse "transaction range too wide"
            (iEnd - iStart #<= piLength)
        #&& ptraceIfFalse "cycle not within bounds"
            (0 #<= cycleN #&& cycleN #<= maxCycles)
        #&& ptraceIfFalse "transaction range starts too soon"
            (piStartOffset #<= iStartOff)
        #&& ptraceIfFalse "transaction range starts too late"
            (iEndOff #<= iEndOff)
    where getTime ::
            Term s (PExtended PPOSIXTime) -> TermCont s (Term s PPOSIXTime)
          getTime x = pure $ pmatch x $ \case
            PFinite n -> pfield @"_0" # n
            _ -> ptraceError "error when getting PPOSIXTime from interval"

pext :: forall (s :: S) (a :: PType) .
  PIsData a =>
  Term s a -> Term s (PAsData (PExtended a))
