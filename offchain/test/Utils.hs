...
import Ledger (ChainIndexTxOut, Datum (Datum), PaymentPubKeyHash (PaymentPubKeyHash), PubKeyHash (PubKeyHash), TxOut)
...
import Plutus.V1.Ledger.Api (Address, BuiltinByteString, BuiltinData, CurrencySymbol, FromData, POSIXTime, POSIXTimeRange, Script, ToData (toBuiltinData), TokenName (TokenName), TxOut (TxOut), TxOutRef, Value, adaSymbol, adaToken, singleton, toData)
...
import SingularityNet.Types (AssetClass (AssetClass), BondedPoolParams (..), BondedStakingAction (AdminAct, StakeAct), BondedStakingDatum (AssetDatum, EntryDatum, StateDatum), Entry (..), ListAction (ListInsert), MintingAction (MintHead))
import Test.Plutip.Contract (TestWallets, initAda)
import Types (BondedPoolScripts (..), BondedPoolTypedScripts (..), InitialBondedPoolParams (..), TBondedPool (TBondedPool, tbpStateDatum, tbpTxOutRef), TEntry (TEntry, teEntryDatum, teTxOutRef))
...
findUtxosWithDatum x = Map.filter $
  \TxOut {txOutDatumHash} -> case txOutDatumHash of
    ...
    Nothing -> False
  where
    hash = Ledger.datumHash . Datum . toBuiltinData
...
createBondedPool scripts initialParams = do
  ...
  poolUtxo <-
    liftContractM "createBondedPool: Could not get new UTxO of the pool" $
    ...
    case tbpStateDatum of
      ...
      StateDatum Nothing -> do
        ...
        unless (minStake <= stakeAmt && stakeAmt <= maxStake) $
          Contract.throwError
            "userHeadStake: stake amount too \
            \large/small"
        let -- Prepare new entry
            entry =
              Entry
                { key = userKey
                , newDeposit = stakeAmt
        ...
        newPoolUtxo <-
          liftContractM
            "userHeadStake: Could not get new UTxO\
            \ of the pool"
            $ findPoolUtxo bpp txOutputs
        entryUtxo <-
          liftContractM
        ...
        pure
          ( TBondedPool {tbpTxOutRef = newPoolUtxo, tbpStateDatum = newState}
...
poolDeposit
  ...
  (Natural depositAmtNat) = do
    ...
    newPoolUtxo <-
      liftContractM
        "poolDeposit: Could not get new UTxO\
        \ of the pool"
        $ findPoolUtxo bpp txOutputs
    entryUtxo <-
      liftContractM
        ...
        $ findEntryUtxo bpp userKey txOutputs
    pure
      ( TBondedPool {tbpTxOutRef = newPoolUtxo, tbpStateDatum}
      , [TEntry {teTxOutRef = entryUtxo, teEntryDatum = EntryDatum entry}]
      )
